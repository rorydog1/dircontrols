...
/**
Changes for CLARA
AnalogData read moved out of inner loop for speed
P_PERSCANNED and percentScan added
*/
...
public:
    MV2(char * name, char * port, char *userial = "NONE");
    ~MV2();
...
private:
    ...
    int        percentScanned_;
    void       setPercentScanned(int i, int l);
    epicsInt32 getPercentScanned();
...
MV2::MV2(char * name, char *address, char *userial)
 :  asynPortDriver(name,
        1,              /* maxAddr */
    ...
    sumP_(0.0)
{
    ...
    createParam("PERSCANNED", asynParamInt32, &P_PERSCANNED);
                ...
                if (event == "Sensors")
                {
                    ...
                    if (sscanf(buffer, "Sensors %s\r\n  ", okBuffer))
                    {
                        ...
                        if (std::string(okBuffer) == "OK")
                        {
                            ...
                            //rfc: 2-4 options are only used for simulations with VM RGAs, they are not used in production
                            char state[SMALL_BUFFER_SIZE];
                            char serialNumber[SMALL_BUFFER_SIZE];
                            char name[SMALL_BUFFER_SIZE];

                            char state2[SMALL_BUFFER_SIZE];
                            char serialNumber2[SMALL_BUFFER_SIZE];
                            char name2[SMALL_BUFFER_SIZE];

                            char state3[SMALL_BUFFER_SIZE];
                            char serialNumber3[SMALL_BUFFER_SIZE];
                            char name3[SMALL_BUFFER_SIZE];

                            char state4[SMALL_BUFFER_SIZE];
                            char serialNumber4[SMALL_BUFFER_SIZE];
                            char name4[SMALL_BUFFER_SIZE];

                            int numScanned = sscanf(buffer,
                                "Sensors %*s\r\n  State  SerialNumber   Name\r\n  %s  %s %s\r\n  %s  %s  %s\r\n  %s  %s %s\r\n\r\n  %s  %s %s",
                                state , serialNumber ,name,
                                state2, serialNumber2,name2,
                                state3, serialNumber3,name3,
                                state4, serialNumber4,name4
                                );
                            float total_rga=numScanned/3;
                            printf("Found %d RGAs \n", int(total_rga) );
                            const int NUM_SCANNED_EXPECTED = 3;
                            if (numScanned == NUM_SCANNED_EXPECTED || numScanned == NUM_SCANNED_EXPECTED*4)
                            {
                                ...
                                {// Simulation mode
                                 //Match one of the sensors
                                  asynPrint(this->pasynUserSelf, ASYN_TRACE_FLOW,
                                    "(%s) Sensor: State:%s SN:%s Name:%s\n", 
                                    portName, state, serialNumber, name);
                                   if (serial_number.find(serialNumber)  != std::string::npos){                                   
                                    SensorInfo sensor = { state, serialNumber, name };
                                    sensors_.push_back(sensor);
                                    printf("RGA1 to: %s \n", serial_number.c_str());
                                    serial_number="FOUND";
                                   ...
                                   if (serial_number.find(serialNumber2) != std::string::npos){
                                    SensorInfo sensor = { state2, serialNumber2, name2 };
                                    sensors_.push_back(sensor);
                                    printf("RGA2 to: %s \n", serial_number.c_str());
                                    serial_number="FOUND";
                                   };

                                   if (serial_number.find(serialNumber3) != std::string::npos){
                                    SensorInfo sensor = { state3, serialNumber3, name3 };
                                    sensors_.push_back(sensor);
                                    printf("RGA3 to: %s \n", serial_number.c_str());
                                    serial_number="FOUND";
                                   };

                                   if (serial_number.find(serialNumber4) != std::string::npos){
                                    SensorInfo sensor = { state4, serialNumber4, name4 };
                                    sensors_.push_back(sensor);
                                    printf("RGA4 to: %s \n", serial_number.c_str());
                                    serial_number="FOUND";
                                   };

                                   if (serial_number.find("FOUND") == std::string::npos){
                                    // No matching serial number found
                                    asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
                                    "(%s) No valid sensor info\n", portName);
                                     printf("No valid RGA simulation serial number found for: %s \n", serial_number.c_str());
                                   };
...
void MV2::handleMassReading(float mass, double reading)
{
    ...
    setPercentScanned( (int)index, (int) lastIndex(headState_.status()) );
...
asynStatus MV2::readInt32(asynUser *pasynUser, epicsInt32 * value)
{
    ...
    if (reason == P_PERSCANNED)
    {
      newValue = getPercentScanned();
    }

    else if (reason == P_DATA)
    {
...
void MV2::setPercentScanned(int i, int l)
{
if(l > 0){
 percentScanned_ =  (int)round(100.0*i/l);
}
}

epicsInt32 MV2::getPercentScanned()
{
return (epicsInt32)percentScanned_;
}
...
extern "C"
{
    static const iocshArg InitArg[] = { { "name", iocshArgString } , { "port", iocshArgString }, { "vmserial", iocshArgString}};
    static const iocshArg * const InitArgs[] = { &InitArg[0], &InitArg[1], &InitArg[2] };
    static const iocshFuncDef InitFuncDef = {"mv2init", 3, InitArgs};

    static void InitCallFunc(const iocshArgBuf * args)
    {
        printf("InitCallFunc %s %s %s\n", args[0].sval, args[1].sval, args[2].sval);
        new rgamv2::MV2(args[0].sval, args[1].sval, args[2].sval);
    }
